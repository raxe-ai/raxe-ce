...
> **Note:** Disabling telemetry requires **Pro tier or higher**. Community Edition (free) users help improve detection by contributing anonymized metadata.

```yaml
# ~/.raxe/config.yaml (Pro+ only)
telemetry:
  enabled: false  # Pro+ only - disable cloud telemetry
