...
| `endpoint` | string | `https://telemetry.raxe.ai/v1/events` | Telemetry endpoint URL |
...
**Privacy Note:** RAXE CE telemetry only sends detection metadata (rule_id, severity, confidence, L1/L2 hits, timestamp, version). We NEVER send prompts, responses, matched text, or hashes. See [Privacy](#privacy--telemetry) section below.
...
export RAXE_TELEMETRY_ENDPOINT=https://telemetry.raxe.ai/v1/events
...
RAXE CE ships with telemetry **enabled by default** to help improve community detection quality. We have **zero tolerance for identifiable data**.

**What we SEND (Detection Metadata Only):**
```json
{
  "rule_id": "pi-001",
  "severity": "HIGH",
...
**What we NEVER send:**
- ❌ Prompts or responses (never transmitted)
- ❌ Matched text or snippets (never transmitted)
- ❌ Hashes of prompts/text (hashes can be reversed)
- ❌ Rule patterns or detection logic (privacy-first)
...
# View telemetry events (local copy)
cat ~/.raxe/telemetry.log

# Scan with telemetry disabled
raxe scan "test" --no-telemetry
```

All telemetry code is open source at `src/raxe/infrastructure/telemetry/` - audit it yourself!
