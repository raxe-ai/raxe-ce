...
class TestL2OnlyBlocking:
    ...
    def test_policy_blocks_on_l2_critical_threat(self, tmp_path):
        ...
        # Create policy that blocks L2 semantic jailbreak (CRITICAL)
        policy = Policy(
            policy_id="test-block-l2-critical",
            customer_id="test-customer",
            name="Block L2 CRITICAL",
            description="Block L2 semantic jailbreak detections",
            conditions=[
                PolicyCondition(
                    rule_ids=["l2-semantic-jailbreak"],  # L2 virtual rule ID
        ...
        for prediction in result.scan_result.l2_result.predictions:
            ...
            assert prediction.threat_type == L2ThreatType.SEMANTIC_JAILBREAK
    ...
    def test_policy_blocks_on_l2_high_threat_when_configured(self, tmp_path):
        ...
        # Policy that blocks on HIGH severity for L2 encoded injection
        policy = Policy(
            policy_id="test-block-l2-high",
            customer_id="test-customer",
            name="Block L2 HIGH",
            description="Block L2 encoded injection (HIGH severity)",
            conditions=[
                PolicyCondition(
                    rule_ids=["l2-encoded-injection"],  # L2 virtual rule ID for ENCODED_INJECTION
        ...
        for detection in result.scan_result.l1_detections:
            ...
            if detection.rule_id == "l2-encoded-injection":
    ...
    def test_policy_does_not_block_on_l2_high_when_not_configured(self, tmp_path):
        ...
        allow_high_policy = Policy(
            ...
            customer_id="test-customer",
            name="Allow L2 HIGH",
            description="Allow L2 context manipulation (HIGH severity)",
            conditions=[
                PolicyCondition(
        ...
        for detection in result.scan_result.l1_detections:
            ...
            if detection.rule_id == "l2-context-manipulation":
