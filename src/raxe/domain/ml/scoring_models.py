...
class ThreatLevel(Enum):
    ...
    - LIKELY_THREAT: High confidence threat with obvious attack patterns (auto-block + spot-check)
...
class ActionType(Enum):
    ...
    - BLOCK_WITH_REVIEW: Block request but add to spot-check queue (LIKELY_THREAT classification)
...
class ThreatScore:
    ...
    Contains the three-level hierarchical classification:
    1. Binary: Threat vs Safe
    2. Family: Broad attack category (PI, JB, TOX, XX, etc.)
    3. Subfamily: Specific attack type within family
    ...
    Attributes:
        ...
        subfamily_confidence: Confidence in predicted subfamily (0.0-1.0)
        binary_proba: Full binary probability distribution [safe, threat]
        family_proba: Full family probability distribution (sorted by index)
        subfamily_proba: Full subfamily probability distribution (sorted by index)
        family_name: Optional predicted family name (e.g., "PI", "JB")
        subfamily_name: Optional predicted subfamily name (e.g., "pi_instruction_override")
    ...
    Example:
        ...
        ...     subfamily_proba=[0.439, 0.3, 0.15, 0.08, 0.02, 0.008, 0.003]
        ... )
    """
    ...
    def __post_init__(self) -> None:
        ...
        if len(self.subfamily_proba) < 2:
            raise ValueError(f"subfamily_proba must have at least 2 elements, got {len(self.subfamily_proba)}")
        if not all(0.0 <= p <= 1.0 for p in self.subfamily_proba):
