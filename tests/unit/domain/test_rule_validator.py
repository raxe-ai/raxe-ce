...
class TestRuleValidator:
    ...
    def test_validate_docs_url(self, validator, valid_rule_data):
        ...
        with tempfile.NamedTemporaryFile(mode="w", suffix=".yaml", delete=False) as f:
            ...
            warnings = [
                i for i in result.issues if "docs_url" in i.field and i.severity == "warning"
            ]
            assert len(warnings) > 0
    ...
    def test_validate_low_confidence(self, validator, valid_rule_data):
        ...
        with tempfile.NamedTemporaryFile(mode="w", suffix=".yaml", delete=False) as f:
            ...
            warnings = [
                i for i in result.issues if "confidence" in i.field and i.severity == "warning"
            ]
            assert len(warnings) > 0
    ...
    def test_validate_rule_id_format(self, validator, valid_rule_data):
        ...
        with tempfile.NamedTemporaryFile(mode="w", suffix=".yaml", delete=False) as f:
            ...
            warnings = [
                ...
                if "rule_id" in i.field and "convention" in i.message.lower()
            ]
            assert len(warnings) > 0
    ...
    def test_validate_pattern_timeout(self, validator, valid_rule_data):
        ...
        with tempfile.NamedTemporaryFile(mode="w", suffix=".yaml", delete=False) as f:
            ...
            warnings = [
                i for i in result.issues if "timeout" in i.field and "short" in i.message.lower()
            ]
            assert len(warnings) > 0
        ...
        with tempfile.NamedTemporaryFile(mode="w", suffix=".yaml", delete=False) as f:
            ...
            warnings = [
                i for i in result.issues if "timeout" in i.field and "long" in i.message.lower()
            ]
            assert len(warnings) > 0
    ...
    def test_validate_mitre_attack_format(self, validator, valid_rule_data):
        ...
        with tempfile.NamedTemporaryFile(mode="w", suffix=".yaml", delete=False) as f:
            ...
            errors = [
                i
                for i in result.issues
                if "mitre" in i.message.lower() or "mitre_attack" in i.field.lower()
            ]
            assert len(errors) > 0
