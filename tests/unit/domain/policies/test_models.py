...
class TestPolicy:
    ...
    def test_minimal_policy(self):
        ...
        assert len(policy.notify_webhooks) == 0
    ...
    def test_full_policy(self):
        ...
        policy = Policy(
            ...
            priority=100,
            notify_webhooks=["https://example.com/webhook"],
            enabled=False,
            metadata={"created_by": "admin"},
        )
        assert policy.override_severity == Severity.CRITICAL
        assert policy.priority == 100
        assert policy.notify_webhooks == ["https://example.com/webhook"]
    ...
    def test_webhook_validation(self):
        ...
        Policy(
            ...
            conditions=[PolicyCondition()],
            action=PolicyAction.BLOCK,
            notify_webhooks=["https://example.com/hook"],
        )
...
class TestPolicyDecision:
    ...
    def test_minimal_decision(self):
        ...
        assert len(decision.notify_webhooks) == 0
    ...
    def test_full_decision(self):
        ...
        decision = PolicyDecision(
            ...
            notify_webhooks=["https://example.com/hook1", "https://example.com/hook2"],
            metadata={"reason": "test"},
        )
        assert decision.matched_policies == ["policy-001", "policy-002"]
        assert len(decision.notify_webhooks) == 2
    ...
    def test_has_notifications(self):
        """has_notifications property."""
        assert PolicyDecision(
            action=PolicyAction.BLOCK,
            original_severity=Severity.HIGH,
            final_severity=Severity.HIGH,
            notify_webhooks=["https://example.com/hook"],
        ).has_notifications is True

        assert PolicyDecision(
            action=PolicyAction.BLOCK,
            original_severity=Severity.HIGH,
            final_severity=Severity.HIGH,
        ).has_notifications is False

    def test_webhook_deduplication(self):
        """Duplicate webhooks are removed."""
        decision = PolicyDecision(
            action=PolicyAction.BLOCK,
            original_severity=Severity.HIGH,
            final_severity=Severity.HIGH,
            notify_webhooks=[
                ...
                "https://example.com/hook1",  # Duplicate
            ],
        )
        # Should have only 2 unique webhooks, order preserved
        assert decision.notify_webhooks == [
            "https://example.com/hook1",
            "https://example.com/hook2",
        ]
    ...
    def test_to_dict(self):
        ...
        assert data["notify_webhooks"] == ["https://example.com/hook"]
