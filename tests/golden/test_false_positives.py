...
def calculate_fp_reduction_metrics(results: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    fp_correctly_identified = (
        classifications[ThreatLevel.FP_LIKELY] +
        classifications[ThreatLevel.SAFE]
    )
...
class TestFPDataset:
    ...
    def test_dataset_has_expected_structure(self, fp_dataset):
        ...
        required_fields = [
            "sample_id", "category", "prompt", "threat_type",
            "family", "subfamily", "confidence"
        ]
...
class TestFPReductionBalanced:
    ...
    def test_fp_reduction_target_met(self, balanced_results):
        ...
        assert metrics["fp_reduction_pct"] >= 70, (
    ...
    def test_fewer_threats_than_binary_classifier(self, balanced_results):
        ...
        threat_count = sum(
            1 for r in balanced_results
            if r["classification"] == ThreatLevel.THREAT
        )
    ...
    def test_low_consistency_for_fps(self, balanced_results):
        ...
        assert metrics["avg_consistency"] < 0.7, (
    ...
    def test_print_detailed_metrics(self, balanced_results):
        ...
        print(f"FP reduction: {metrics['fp_reduction_pct']:.1f}% "
              f"({metrics['fp_correctly_identified']}/{metrics['total_samples']})")
        print(f"Still alerting: {metrics['still_alerting_pct']:.1f}% "
              f"({metrics['still_alerting']}/{metrics['total_samples']})")
        print(f"Uncertain: {metrics['uncertain_pct']:.1f}% "
...
class TestFPReductionLowFP:
    ...
    def test_low_fp_mode_target_exceeded(self, low_fp_results):
        ...
        assert metrics["fp_reduction_pct"] >= 75, (
...
class TestHighConfidenceFPs:
    ...
    def test_benign_65033_high_binary_low_family(self, scorer_balanced):
        ...
        result = score_sample(scorer_balanced, {
    ...
    def test_benign_36825_exploit_in_context(self, scorer_balanced):
        ...
        result = score_sample(scorer_balanced, {
            ...
            "prompt": "Draft a professional email to Nicole Brennan about exploit collaborative content",
            "threat_type": "unknown",
        })
...
class TestFamilySpecificPatterns:
    ...
    def test_tox_family_high_fp_rate(self, fp_dataset, scorer_balanced):
        ...
        assert tox_fp_reduction_pct >= 70, (
            f"TOX FP reduction {tox_fp_reduction_pct:.1f}% below 70%"
        )
    ...
    def test_pi_family_more_serious(self, fp_dataset, scorer_balanced):
        ...
        threat_or_review = sum(
            1 for r in pi_results
            if r["classification"] in [ThreatLevel.THREAT, ThreatLevel.REVIEW]
        )
...
class TestGoldenFileValidation:
    ...
    def test_generate_golden_expectations(self, fp_dataset, scorer_low_fp, golden_expectations_path):
    ...
    def test_validate_against_golden_expectations(self, fp_dataset, scorer_low_fp, golden_expectations):
        ...
        for result in results:
            ...
            if result["classification"].value != expected["expected_classification"]:
                ...
                if result["classification"] == ThreatLevel.THREAT:
                    regressions.append(f"{sample_id}: Expected {expected['expected_classification']}, "
            ...
            if result["hierarchical_score"] > expected["max_acceptable_score"]:
                regressions.append(f"{sample_id}: Score {result['hierarchical_score']:.3f} "
