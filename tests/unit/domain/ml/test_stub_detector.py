...
class TestStubL2Detector:
    ...
    def test_detect_base64_encoded_content(self):
        ...
        # Stub detector uses ENCODING_OR_OBFUSCATION for encoded content
        encoded_preds = result.get_predictions_by_type(L2ThreatType.ENCODING_OR_OBFUSCATION)
    ...
    def test_detect_encoding_keywords(self):
        ...
        assert result.has_predictions
        # Stub detector uses ENCODING_OR_OBFUSCATION for encoding keywords
        encoded_preds = result.get_predictions_by_type(L2ThreatType.ENCODING_OR_OBFUSCATION)
        assert len(encoded_preds) > 0
    ...
    def test_detect_code_execution_patterns(self):
        ...
        assert result.has_predictions
        # Stub detector uses JAILBREAK for code execution patterns
        code_preds = result.get_predictions_by_type(L2ThreatType.JAILBREAK)
    ...
    def test_detect_privilege_escalation(self):
        ...
        assert result.has_predictions
        # Stub detector maps privilege escalation to JAILBREAK
        priv_preds = result.get_predictions_by_type(L2ThreatType.JAILBREAK)
    ...
    def test_detect_obfuscation(self):
        ...
        # Should detect obfuscation - maps to ENCODING_OR_OBFUSCATION
        obf_preds = result.get_predictions_by_type(L2ThreatType.ENCODING_OR_OBFUSCATION)
        # Note: May or may not trigger depending on patterns
        # The test validates the code runs without error
    ...
    def test_context_manipulation_requires_l1_detections(self):
        ...
        ctx_preds = result.get_predictions_by_type(L2ThreatType.RAG_OR_CONTEXT_ATTACK)
    ...
    def test_context_manipulation_with_l1_and_long_text(self):
        ...
        # Should NOW detect context manipulation - maps to RAG_OR_CONTEXT_ATTACK
        ctx_preds = result.get_predictions_by_type(L2ThreatType.RAG_OR_CONTEXT_ATTACK)
        assert len(ctx_preds) > 0
