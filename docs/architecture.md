...
│  • ThreatDetector (pure detection logic)                        │
...
2. **One-Way Hashing** - SHA-256 hashes are irreversible
3. **Optional Telemetry** - Disabled by default, user controls everything
4. **No PII Collection** - System designed to never see PII
5. **Offline Capable** - Works 100% offline with zero degradation
