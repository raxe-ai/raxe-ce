...
raxe config set l2_enabled true
```
...
result.highest_severity      # Severity - highest severity found
result.duration_ms          # float - scan time in milliseconds
result.scan_result          # ScanResult object with detections
...
async def generate(prompt: str):
    ...
    if result.has_threats:
        raise HTTPException(
            status_code=400,
            detail=f"Security threat detected: {result.highest_severity}"
        )
