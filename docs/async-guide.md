...
async with AsyncRaxe() as raxe:
    ...
    results = await raxe.scan_batch(
        prompts,
        max_concurrent=50  # Prevents overwhelming system
    )
```
...
**scan_batch(texts: List[str], max_concurrent: int = 10) -> List[ScanPipelineResult]**
...
| Concurrent scans (max_concurrent=100) | ~5,000 req/sec |
    ...
    try:
        ...
        if scan_result.scan_result.has_threats:
            logger.warning(
                ...
                f"{scan_result.scan_result.combined_severity}"
            )
            raise HTTPException(
                status_code=400,
