...
```yaml
telemetry:
  enabled: false  # Pro+ only - disable telemetry
...
Scan multiple prompts together using AsyncRaxe:
...
# Sequential: Individual scans (simple but slower)
raxe = Raxe()
for prompt in prompts:
    result = raxe.scan(prompt)

# Parallel: Batch scan with AsyncRaxe (faster for large batches)
async_raxe = AsyncRaxe()
results = await async_raxe.scan_batch(prompts, max_concurrency=10)
```

**Impact**: 5-10x faster for batches > 10 prompts using async
