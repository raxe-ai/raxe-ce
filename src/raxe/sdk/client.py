...
class Raxe:
    ...
    def _track_scan(
        ...
        """Track scan telemetry (non-blocking, never raises).

        This method sends privacy-preserving telemetry about the scan:
        ...
        try:
            ...
            if orchestrator is None:
                return
            # Don't check is_enabled() here - let track_scan() handle initialization
            # is_enabled() returns False before initialization, blocking the lazy init

            # Build scan result dictionary for telemetry
            # Only includes privacy-safe metadata
            scan_telemetry: dict[str, Any] = {
                "threat_detected": result.has_threats,
                "detection_count": result.total_detections,
                "highest_severity": result.severity if result.has_threats else None,
                "rule_ids": [],
                "families": [],
                "l1_hit": False,
                "l2_hit": False,
                "l2_enabled": result.metadata.get("l2_enabled", True),
                "prompt_length": len(result.text_hash) if result.text_hash else 0,
                "action_taken": "block" if result.should_block else "allow",
            }

            # Extract rule IDs and families from L1 detections
            if result.scan_result and result.scan_result.l1_result:
                l1_result = result.scan_result.l1_result
                scan_telemetry["l1_hit"] = l1_result.detection_count > 0
                for detection in l1_result.detections:
                    if hasattr(detection, "rule_id"):
                        scan_telemetry["rule_ids"].append(detection.rule_id)
                    if hasattr(detection, "family"):
                        family = detection.family
                        if family and family not in scan_telemetry["families"]:
                            scan_telemetry["families"].append(family)

            # Extract L2 hit status
            if result.scan_result and result.scan_result.l2_result:
                l2_result = result.scan_result.l2_result
                scan_telemetry["l2_hit"] = getattr(l2_result, "is_threat", False)

            # Get L1/L2 durations from result metadata if available
            l1_duration_ms = None
            l2_duration_ms = None
            if result.scan_result:
                if result.scan_result.l1_result:
                    l1_duration_ms = result.scan_result.l1_result.scan_duration_ms
                # L2 duration would be in metadata or l2_result

            orchestrator.track_scan(
                scan_result=scan_telemetry,
        ...
        *,
        ...
        if not dry_run:
            try:
                ...
                self._track_scan(result, entry_point="sdk", event_id=event_id)
