...
- L2 prediction → virtual rule ID mapping (e.g., "l2-prompt-injection")
...
class TestL2VirtualRuleMapping:
    ...
    def test_prompt_injection_virtual_rule_id(self):
        """L2 prompt injection maps to 'l2-context-manipulation'."""
        ...
        assert detections[0].rule_id == "l2-context-manipulation"
        assert detections[0].category == "context_manipulation"
        assert detections[0].detection_layer == "L2"

    def test_jailbreak_virtual_rule_id(self):
        """L2 jailbreak maps to 'l2-semantic-jailbreak'."""
        prediction = L2Prediction(
        ...
        assert len(detections) == 1
        assert detections[0].rule_id == "l2-semantic-jailbreak"
        assert detections[0].category == "semantic_jailbreak"

    def test_threat_type_with_underscores(self):
        """Threat type with underscores converted to hyphens."""
        prediction = L2Prediction(
        ...
        assert len(detections) == 1
        assert detections[0].rule_id == "l2-data-exfil-pattern"
        assert detections[0].category == "data_exfil_pattern"

    def test_threat_type_lowercase_conversion(self):
        """Threat type converted to lowercase for rule ID."""
        prediction = L2Prediction(
            threat_type=L2ThreatType.PRIVILEGE_ESCALATION,
            confidence=0.90,
        )
        l2_result = L2Result(
            predictions=[prediction],
        ...
        assert len(detections) == 1
        assert detections[0].rule_id == "l2-privilege-escalation"
    ...
    def test_multiple_predictions_create_multiple_detections(self):
        ...
        assert rule_ids == {"l2-context-manipulation", "l2-semantic-jailbreak", "l2-encoded-injection"}
...
class TestL2VirtualDetectionCreation:
    ...
    def test_virtual_detection_has_correct_structure(self):
        ...
        assert detection.rule_id == "l2-context-manipulation"
        assert detection.rule_version == "1.0.0"
        assert detection.category == "context_manipulation"
    ...
    def test_virtual_detection_metadata_includes_threat_type(self):
        ...
        assert detection.category == "semantic_jailbreak"
        assert detection.detection_layer == "L2"
...
class TestL2VirtualRulesInPolicyMatching:
    ...
    def test_virtual_rule_id_format_matches_policy_pattern(self):
        ...
        threat_types = [
            L2ThreatType.CONTEXT_MANIPULATION,
            L2ThreatType.SEMANTIC_JAILBREAK,
            L2ThreatType.ENCODED_INJECTION,
            L2ThreatType.DATA_EXFIL_PATTERN,
            L2ThreatType.OBFUSCATED_COMMAND,
        ]
        ...
        for threat_type in threat_types:
            ...
            assert "_" not in rule_id  # Underscores converted to hyphens
