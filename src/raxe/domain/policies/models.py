...
class Policy:
    ...
    Attributes:
        ...
        notify_webhooks: Webhook URLs to notify on match (empty = no notifications)
    ...
    def __post_init__(self) -> None:
        ...
        # Validate webhook URLs (basic validation)
        for webhook_url in self.notify_webhooks:
            if not webhook_url.startswith(("http://", "https://")):
                raise ValueError(
                    f"Invalid webhook URL (must start with http:// or https://): {webhook_url}"
                )
...
class PolicyDecision:
    ...
    Attributes:
        ...
        notify_webhooks: Aggregated webhook URLs from all matched policies
    ...
    def __post_init__(self) -> None:
        """Validate decision."""
        # Remove duplicate webhooks while preserving order
        object.__setattr__(
            self,
            'notify_webhooks',
            list(dict.fromkeys(self.notify_webhooks))
        )
    ...
    def has_notifications(self) -> bool:
        """True if webhooks should be notified."""
        return len(self.notify_webhooks) > 0
    ...
    def to_dict(self) -> dict[str, object]:
        ...
        return {
            ...
            "notify_webhooks": self.notify_webhooks,
