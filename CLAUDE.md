...
Quick Feature → fullstack-dev
Bug → appropriate dev agent
Customer Issue → support-engineer

####Practical Workflows**
Standard: product-owner → tech-lead → dev → qa → devops
Quick Fix: support → fullstack-dev → qa → devops
ML Feature: tech-lead → ml-engineer → backend-dev → qa
...
####TASKS.md
Complete LinearB Backlog - You should use this as a tracker for backlog, new tasks, complete tasks, epics, user journies, stories, tasks
...
### When Implementing Features after
1. **New Detection Logic** → Add to domain layer (pure functions)
2. **New API Endpoint** → Add rate limiting + auth
3. **Performance Impact** → Run benchmarks
4. **Security Sensitive** → Extra review + audit log
5. **User-Facing Change** → Update README + CHANGELOG
6. **Architecture Change** → Update this CLAUDE.md and Docs
...
### Internal Documentation to keep updated
