...
def scan_progressive(pipeline: ScanPipeline, text: str):
    ...
    if result.has_threats and result.scan_result.combined_severity.value in ["CRITICAL", "HIGH"]:
        return result

    # If no clear threat, run thorough scan
    if not result.has_threats or max(d.confidence for d in result.scan_result.l1_result.detections) < 0.7:
